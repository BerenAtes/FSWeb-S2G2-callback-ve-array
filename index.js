...
function Yillar(data, finallerCallback) {
  let finalMaclari = finallerCallback(data);
  const years = finalMaclari.map((matches) => matches.Year);
  return years;
}
...
function Kazananlar(data, finallerCallback) {
  const finalMaclari = finallerCallback(data);
  const kazananTakim = finalMaclari.map((match) => {
    if (match["Home Team Goals"] > match["Away Team Goals"]) {
      return match["Home Team Name"];
    } else if (match["Home Team Goals"] > match["Away Team Goals"]) {
      return match["Away Team Goals"];
    } else {
      return "Berabere!";
    }
  });
...
console.log("Kazanan Ülke İsimleri:", Kazananlar(fifaData, Finaller));
...
function YillaraGoreKazananlar(
  ...
  const kazananTakim = kazananlarCallback(data, finallerCallback);
  let sayi;
  let kazananlar = [];
  let sonuclar = [];
  for (let i = 0; 0 < years.length; i++) {
    sayi = years[i];
    kazananlar = kazananTakim[i];
